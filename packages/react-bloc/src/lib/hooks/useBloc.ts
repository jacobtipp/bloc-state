...
>(
  ...
  const selectedState = useBlocSelector(bloc, config); // Retrieve the selected or derived state using useBlocSelector.
  const providedBloc = useBlocInstance(bloc); // Retrieve the instance of the Bloc using useBlocInstance.

  return [selectedState, providedBloc] as const; // Return both the selected state and the Bloc instance as a readonly tuple.
};
