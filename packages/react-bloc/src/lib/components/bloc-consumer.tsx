...
export function BlocConsumer<Bloc extends ClassType<BlocBase<any>>>({
  ...
  // Utilize the useBlocListener hook to listen for events on the Bloc based on provided conditions.
  useBlocListener(bloc, {
    listener,
    listenWhen,
  });

  // Return a BlocBuilder component to rebuild the UI based on the Bloc's state changes.
  return BlocBuilder({
    bloc,
    builder,
    buildWhen,
